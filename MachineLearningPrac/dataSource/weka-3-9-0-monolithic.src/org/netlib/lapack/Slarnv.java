  ...
  public static void slarnv(int paramInt1, int[] paramArrayOfInt, int paramInt2, int paramInt3, float[] paramArrayOfFloat, int paramInt4)
  {
    ...
    for (int n = (paramInt3 - 1 + '' / 2) / ('' / 2); n > 0; n--)
    {
      ...
      else if ((paramInt1 != 3 ? 0 : 1) != 0)
      {
        ...
        for (;;)
        {
          paramArrayOfFloat[(m + i - 1 - 1 + paramInt4)] = ((float)Math.sqrt(-(2.0F * (float)Math.log(arrayOfFloat[(2 * i - 1 - 1)]))) * (float)Math.cos(6.283186F * arrayOfFloat[(2 * i - 1)]));
          i += 1;
          i1--;
          if (i1 <= 0) {
            break;
 ...
 * Qualified Name:     org.netlib.lapack.Slarnv
